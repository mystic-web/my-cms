...
export default function FAQsPage() {
  ...
  const fetchItems = () => fetch('/api/faqs').then(r => r.json()).then(d => { setItems(d); setLoading(false) })
  ...
  return (
    <div className="p-8 animate-fadeIn">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-display text-3xl text-ink-900 font-bold">FAQs</h1>
          <p className="text-ink-500 font-body mt-1">Manage faqs entries</p>
        </div>
        <Link href="/admin/faqs/new" className="bg-accent hover:bg-accent-hover text-white font-body font-semibold px-5 py-2.5 rounded transition-colors">
          + Add New
        </Link>
      </div>
      {loading ? (
        ...
        <div className="bg-white rounded-lg border border-ink-200 p-12 text-center">
          ...
          <h3 className="font-display text-xl text-ink-700 mb-2">No faqs yet</h3>
          <Link href="/admin/faqs/new" className="bg-accent text-white font-body px-5 py-2.5 rounded inline-block mt-3">
            Add First
          </Link>
        </div>
      ) : (
                ...
                <tr key={item._id} className="border-b border-ink-100 hover:bg-ink-50 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      <Link href={`/admin/faqs/${item._id}`} className="text-accent hover:underline font-body text-sm">Edit</Link>
